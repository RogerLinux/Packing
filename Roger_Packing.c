...
float cov(float* ctr_x_ref, float* ctr_y_ref, float* ctr_r_ref, float L, float W, float dmin, int NUM){
    ...
    for(j=0;j<L;j+=dmin){
       for(i=0;i<W;i+=dmin){
           for(n=0;n<NUM;n++)if(dist(j, i, ctr_x_ref[n], ctr_y_ref[n])<=ctr_r_ref[n]){
              inrange++;
              break;
                                       }
           total++;
...
float MineDeploy(float* ctr_x_opt, float* ctr_y_opt, float* ctr_x, float* ctr_y, float* R, float L, float W, int NUM, float dmin, float R_th_min, float R_th_MAX, float d_R_th, int iter){
    ...
    float* ctr_x_temp, * ctr_y_temp;

    printf("Loading original location data...\n");

    ctr_x_temp=malloc(NUM*sizeof(float));
    ctr_y_temp=malloc(NUM*sizeof(float));
        ...
        for(n=0;n<NUM;n++){
           ...
           if(cov_now>cov_opt){
                ...
                for(k=0;k<NUM;k++){
                   ...
                   ctr_y_opt[k]=ctr_y_temp[k]; //update the best deployment and coverage if better deployment is found
                   }
                                   }
           R_th-=d_R_th;

           if(R_th<=R_th_min)d_R_th*=-1;
             else if(R_th>=R_th_MAX && d_R_th<0)break;
                               }
        free(ctr_x_temp);
        free(ctr_y_temp);
